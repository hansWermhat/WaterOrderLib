...
def getLSI(subPos, Pos, BoxDims, lowCut=0.0, highCut=3.7):
  """Uses waterlib.f90 library tools and basic tools as described in Shiratani and Sasai, JCP 1996.
     Inputs:
  ...
  #Set-up array to hold angle results and stack as go... list increases in size!
  lsiVals = [] #np.array([])
  numLSI = np.zeros(len(subPos))

  #Find nearest neighbors for ALL atoms in subPos
  #But make sure using efficient algorithm...
  #If subPos is same as Pos, use allnearneighbors instead
  if np.array_equal(subPos, Pos):
    nearNeighbs = wl.allnearneighbors(Pos, BoxDims, 
                                      lowCut, highCut).astype(bool)
    nextNeighbs = wl.allnearneighbors(Pos, BoxDims, highCut, 
                                      highCut+3.7).astype(bool)
  else:
    nearNeighbs = wl.nearneighbors(subPos, Pos, BoxDims, lowCut, 
                                   highCut).astype(bool)
    nextNeighbs = wl.nearneighbors(subPos, Pos, BoxDims, highCut, 
                                   highCut+3.7).astype(bool)

  #Loop over each position in subPos, finding angle made with all neighbor pairs
  for (i, apos) in enumerate(subPos):
    #Make sure have nearest neighbors...
    if len(Pos[nearNeighbs[i]]) > 1:
      if len(Pos[nextNeighbs[i]]) > 0:
        # determine the next nearest neighbor > 3.7
        nextDist = np.sqrt( np.sum( (Pos[nextNeighbs[i]]-apos)**2.0, 
                                    axis=1) )
        nextInd = np.argmin(nextDist)
      
        # add next neighbor to the list of neighbors
        pos = np.vstack((Pos[nearNeighbs[i]], Pos[nextNeighbs[i]][nextInd]))

      else:
        continue

      # compute!
      tempDists = np.sort(wl.lsidists(apos, pos, BoxDims))
      tempDelta = tempDists[1:] - tempDists[:-1]
      tempLSI = np.var(tempDelta)
      
      # Store LSI and number of NN waters
      lsiVals.append(tempLSI) 

      numLSI[i] = len(tempDelta)
    else:
      continue

  return np.array(lsiVals), numLSI
...
def tetrahedralMetrics(angVals, nBins=500, binRange=[0.0, 180.0]):
  ...
  angTet = angVals[np.where( (angVals>=100.0)&(angVals<=120.0) )[0]]

  # compute stats
  fracTet = float(len(angTet))/float(len(angVals))
  avgCos = np.mean( np.cos(angTet*np.pi/180.0) )
  varCos = np.var( np.cos(angTet*np.pi/180.0) )

  # compute entropy
  angDens = angDist/float(np.sum(angDist))
  angDens = angDens[np.where(angDens!=0)[0]]
  entropy = -np.sum(angDens*np.log(angDens))
  
  return angDist, bins, fracTet, avgCos, varCos, entropy

def getOrderParamq(subPos, Pos, BoxDims, lowCut=0.0, highCut=10.0):
...
tetrahedral order parameter, q, used by Errington and Debenedetti (2001).
  ...
  for (i, apos) in enumerate(subPos):
    ...
    if np.sum(nearNeighbs[i]) > 0:
      ...
      newPos = thisPos[np.argsort(thisDists)][:4]
      #below returns symmetric, square array (zero diagonal)
      tempAng = wl.tetracosang(apos, newPos, BoxDims)
      #Only want half of array, flattened
      angVals = tempAng[np.triu_indices(len(tempAng),k=1)]
      if np.sum(nearNeighbs[i])==1:
        angVals = 180.0*np.ones(6)
      elif np.sum(nearNeighbs[i])==2:
        angVals = np.concatenate((angVals, 180.0*np.ones(5)))
      elif np.sum(nearNeighbs[i])==3:
        angVals = np.concatenate((angVals, 180.0*np.ones(3)))
      else:
        angVals = angVals
...
def getOrderParamPsi(subPos, Pos, BoxDims, lowCut=0.0, highCut=10.0):
  """Finds angles between chain ends and compute hexagonal order parameter (psi) used by Dallin and van Lehn (2019).
     Inputs: 
  ...
  #Loop over each position in subPos, finding angle made with the neighbors, then psi
  for (i, apos) in enumerate(subPos):
    #Make sure have nearest neighbors...
    if np.sum(nearNeighbs[i]) > 1:
      thisPos = wl.reimage(Pos[nearNeighbs[i]], apos, BoxDims)
      thisDists = np.linalg.norm(thisPos - apos, axis=1)
      newPos = thisPos[np.argsort(thisDists)]
      #below returns symmetric, square array (zero diagonal)
      tempAng = wl.tetracosang(apos, newPos, BoxDims)
      #Only want half of array, flattened
      angVals = tempAng[np.triu_indices(len(tempAng),k=1)]
      #Now compute psi for this set of angles
      psiVals[i] = np.mean( np.exp(6j*angVals*np.pi/180.0) )
      psiValsConj = np.conjugate(psiVals[i])
      psiVals[i] = np.sqrt( (psiVals[i]*psiValsConj).real )
