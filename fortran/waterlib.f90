...
! Assumes a bulk density of atoms in Pos2 provided... i.e. system can be inhomogeneous
! rather than assuming homogeneously distributed atoms in Pos2 over whole volume of box
subroutine RadialDistPlane(Pos1, Pos2, binwidth, totbins, BulkDens, BoxL, rdf, NPos2)
    implicit none
    real(8), dimension(3,3), intent(in) :: Pos1
    real(8), dimension(NPos2, 3), intent(in) :: Pos2
    real(8), intent(in) :: binwidth
    integer, intent(in) :: totbins
    ! Total number of bins, so max bin value is totbins*binwidth; if larger ignored
    real(8), intent(in) :: BulkDens
    real(8), dimension(3), intent(in) :: BoxL
    real(8), dimension(totbins,totbins), intent(out) :: rdf
    integer, intent(in) :: NPos2
    external :: crossProd3
    real(8), dimension(3) :: v1, v2, v3
    real(8), dimension(3,3) :: Q
    real(8), parameter :: pi = 3.141592653589D0
    real(8), dimension(totbins,totbins) :: counts
    real(8), dimension(3) :: iPos, iBoxL
    real(8), dimension(3,3) :: newPos1
    real(8) :: distX, distY
    integer :: nbinX, nbinY, i, k, m
    ...
    v1 = Pos1(3,:)-Pos1(1,:)
    v2 = Pos1(2,:)-Pos1(1,:)
    call crossProd3(v1,v2,v3,3)

    v1 = v1 - BoxL * anint(v1 * iBoxL)
    v2 = v2 - BoxL * anint(v2 * iBoxL)
    v3 = v3 - BoxL * anint(v3 * iBoxL)
    
    v2 = v2 - (sum(v1*v2))/(sum(v1**2.0))*v1
    ...
    newPos1(1,:) = matmul(Q,Pos1(1,:)-Pos1(1,:))
    newPos1(2,:) = matmul(Q,Pos1(2,:)-Pos1(1,:))
    newPos1(3,:) = matmul(Q,Pos1(3,:)-Pos1(1,:))
    !print *, Pos1
    !print *, newPos1

    counts(:,:) = 0
    do i = 1, NPos2
        iPos = Pos2(i,:)*BulkDens/BulkDens
        iPos = iPos - BoxL * anint(iPos * iBoxL)
        iPos = matmul(Q,iPos)
        distX = iPos(1)
        distY = iPos(2)
        ! Place in correct bin
        if (iPos(3) <= newPos1(1,3)+5.0 .and. iPos(3) >= newPos1(1,3)-5.0) then
           nbinX = ceiling(distX / binwidth) ! Bin i has all distances less than i but greater than i - 1
           nbinY = ceiling(distY / binwidth)
           
           !print *, iPos(3)

           if (nbinX <= totbins .and. nbinY <= totbins) then
              counts(nbinX,nbinY) = counts(nbinX,nbinY) + 1.
           endif
    ...
    do k = 1, totbins
       do m = 1, totbins
          ...
          rdf(k,m) = counts(k,m) ! / (NPos1 * BulkDens * (4./3.) * pi * (binwidth**3) * ((k**3) - ((k-1)**3)))
       enddo
    enddo
    ! Gives g(r) for single snapshot... for whole trajectory, do for each frame and average
end subroutine
...
! but algorithm here works well if subPos << Pos
! If have subPos = Pos instead use allNearNeighbors
! Distinct from nearNeighbors() because this has low and high cutoff values 
! for each Pos atom
subroutine nearNeighbors2(subPos, Pos, BoxL, lowCut, highCut, NNeighbors, NsubPos, NPos)
    implicit none
    real(8), dimension(NsubPos, 3), intent(in) :: subPos
    real(8), dimension(NPos, 3), intent(in) :: Pos
    real(8), dimension(3), intent(in) :: BoxL
    real(8), intent(in), dimension(NPos) :: lowCut, highCut
    integer, intent(in) :: NsubPos
    integer, intent(in) :: NPos
    logical, dimension(NsubPos, NPos), intent(out) :: NNeighbors
    integer :: i, j
    real(8), dimension(3) :: iBoxL
    real(8), dimension(3) :: pos1, pos2, distvec
    real(8), dimension(NPos) :: lowsq, highsq 
    real(8) :: distvecSq
    iBoxL = merge(1.d0/BoxL, 0.d0, BoxL >= 0.d0) 
    lowsq = lowCut*lowCut
    highsq = highCut*highCut
    ...
    do i = 1, NsubPos
        ...
        do j = 1, NPos
            ...
            distvec  = distvec - BoxL * anint(distvec * iBoxL)
            distvecSq = sum(distvec**2)
            if ( (distvecSq > lowsq(j)) .and. (distvecSq <= highsq(j)) ) then
                ! lower cut-off not included... lazy, really, but prevents including same atom
                NNeighbors(i,j) = .true.
            endif
        enddo
...
! but algorithm here works well if subPos << Pos
! If have subPos = Pos instead use allNearNeighbors
! Distinct from nearNeighbors2() because this will compute the positive 
! (or negative) distances from the SASA for more refined marching_cubes 
! application
subroutine nearNeighbors3(subPos, Pos, BoxL, lowCut, highCut, distMetric, NsubPos, NPos)
    implicit none
    real(8), dimension(NsubPos, 3), intent(in) :: subPos
    real(8), dimension(NPos, 3), intent(in) :: Pos
    real(8), dimension(3), intent(in) :: BoxL
    ...
    integer, intent(in) :: NsubPos
    integer, intent(in) :: NPos
    real(8), dimension(NsubPos, NPos), intent(out) :: distMetric
    integer :: i, j
    real(8), dimension(3) :: iBoxL
    real(8), dimension(3) :: pos1, pos2, distvec
    real(8), dimension(NPos) :: lowsq, highsq 
    ...
    do i = 1, NsubPos
        ...
        do j = 1, NPos
            ...
            distvec  = distvec - BoxL * anint(distvec * iBoxL)
            distvecSq = sum(distvec**2)
            distMetric(i,j) = distvecSq-highsq(j) !.true.
        enddo
    enddo
end subroutine
...
! Compute LSI dists for a specified set 
! Returns all three-body nearest neighbor angles for all combinations of nearest neighbors
! Note that return is just NneighPos x NneighPos symmetric array
subroutine lsiDists(refPos, neighPos, BoxL, dist1, NneighPos)
    implicit none
    real(8), dimension(3), intent(in) :: refPos
    real(8), dimension(NneighPos, 3), intent(in) :: neighPos
    real(8), dimension(3), intent(in) :: BoxL
    integer, intent(in) :: NneighPos
    real(8), dimension(NneighPos), intent(out) :: dist1
    integer :: i
    real(8), dimension(3) :: distvec1, iBoxL
    iBoxL = merge(1.d0/BoxL, 0.d0, BoxL >= 0.d0)
    ! Loop over nearest neighbor positions
    do i = 1, NneighPos
        distvec1 = neighPos(i,:) - refPos
        distvec1 = distvec1 - BoxL * anint(distvec1 * iBoxL)
        ! Loop over other neighbor positions (want combinations)
        dist1(i) = sqrt(sum( distvec1*distvec1 ))
    enddo
...
! than reporting both the dipole and both the OH bond vectors, I think.
! Note that the water dipole direction is just the sum of the OH vectors.
subroutine watOHvec(opos, hpos, BoxL, normdip, Nopos, Nhpos)
    implicit none
    real(8), dimension(Nopos, 3), intent(in) :: opos
    real(8), dimension(Nhpos, 3), intent(in) :: hpos
    real(8), dimension(3), intent(in) :: BoxL
    integer, intent(in) :: Nopos, Nhpos
    real(8), dimension(Nopos,3), intent(out) :: normdip
    real(8), dimension(3) :: vecoh1, vecoh2, vecdip, iBoxL
    integer :: i
    iBoxL = merge(1.d0/BoxL, 0.d0, BoxL >= 0.d0)
    ! Check to make sure number of hydrogens matches oxygens
    if (2*Nopos /= Nhpos) then
    ...
    do i = 1, Nopos
        ...
        vecdip = vecoh1 + vecoh2
        vecdip = vecdip - BoxL * anint(vecdip * iBoxL)
        normdip(i,:) = vecdip/sqrt(sum(vecdip*vecdip))
    enddo
end subroutine
...
! Adapted from the density field calculation below, but this just computes the raw density
...
subroutine DensityField(pos, gridx, gridy, gridz, BoxL, densvals, Npos, Nx, Ny, Nz)
    implicit none
    real(8), dimension(Npos, 3), intent(in) :: pos
    ...
    real(8), parameter :: pi = 3.1415926535897931D0
    real(8), dimension(3) :: iBoxL, apos, watpos, thisvec
    real(8) :: binwidth, densval, rvalsq
    integer :: i, j, k, l
    iBoxL = merge(1.d0/BoxL, 0.d0, BoxL >= 0.d0)
    binwidth = gridx(2)-gridx(1)
        ...
        do j = 1, Ny
            do k = 1, Nz
                ...
                do l = 1, Npos
                    ...
                    if (watpos(1)<(apos(1)-binwidth/2.)) cycle
                    if (watpos(1)>(apos(1)+binwidth/2.)) cycle
                    if (watpos(2)<(apos(2)-binwidth/2.)) cycle
                    if (watpos(2)>(apos(2)+binwidth/2.)) cycle
                    if (watpos(3)<(apos(3)-binwidth/2.)) cycle
                    if (watpos(3)>(apos(3)+binwidth/2.)) cycle
...
!                    if (j.eq.10) stop 123
                ...
                densvals(i, j, k) = densval/(binwidth**3.0)
            enddo
        enddo
    enddo
end subroutine
